...
class Compiler {
    ...
    private static error(message: string, token: Token): never {
        throw new Error(`[line: ${token.line}] ${message}`);
    }
    ...
    private global(): string {
        if (this.match(TokenType.FUNCTION)) {
            ...
            Compiler.error("Expected function or const declaration", this.peek());
        }
    }
    ...
    private literal(): LogoValue {
        if (this.match(TokenType.NUMBER)) {
            return new LogoValue(this.previous().lexeme);
    ...
    private foreachStatement(): string {
        ...
        this.consume(TokenType.NEWLINE, "Expected newline after foreach list");
        const body: string = this.block();
...
const source: string = `function add(x, y) {
    print(x + y)
}`;
const compiled: string = new Compiler(source).compile();
const expected: string = `to add [x y] 
    report x + y
end`;
